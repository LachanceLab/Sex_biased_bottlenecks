...
% number of times data is sampled at the end of each run
number_of_sample_runs=50;

% defines maximum population size and sets bottleneck sizes
max_males=10000;
max_females=10000;
GIH_male_bottleneck=340;
GIH_female_bottleneck=70;
CHB_male_bottleneck=800;
CHB_female_bottleneck=150;
JPT_male_bottleneck=800;
JPT_female_bottleneck=400;
...
JPT_settlement_generation=1200;
generation=1;
...
CHB_male_bottleneck=number_of_males(CHB_settlement_generation);
number_of_females(CHB_settlement_generation)=min(CHB_female_bottleneck,number_of_females(CHB_settlement_generation-1));
CHB_female_bottleneck=number_of_females(CHB_settlement_generation);
for generation=(CHB_settlement_generation+1):1:JPT_settlement_generation
    number_of_males(generation)=round(max_males/(1+((max_males-((CHB_male_bottleneck+CHB_female_bottleneck)/2))/((CHB_male_bottleneck+CHB_female_bottleneck)/2))*exp(-population_growth_rate(generation)*(generation-CHB_settlement_generation-2))));
    number_of_females(generation)=round(max_females/(1+((max_females-((CHB_male_bottleneck+CHB_female_bottleneck)/2))/((CHB_male_bottleneck+CHB_female_bottleneck)/2))*exp(-population_growth_rate(generation)*(generation-CHB_settlement_generation-2))));
end
number_of_males(JPT_settlement_generation)=min(JPT_male_bottleneck,number_of_males(JPT_settlement_generation-1));
JPT_male_bottleneck=number_of_males(JPT_settlement_generation);
number_of_females(JPT_settlement_generation)=min(JPT_female_bottleneck,number_of_females(JPT_settlement_generation-1));
JPT_female_bottleneck=number_of_females(JPT_settlement_generation);
for generation=(JPT_settlement_generation+1):1:max_generation
    number_of_males(generation)=round(max_males/(1+((max_males-((JPT_male_bottleneck+JPT_female_bottleneck)/2))/((JPT_male_bottleneck+JPT_female_bottleneck)/2))*exp(-population_growth_rate(generation)*(generation-JPT_settlement_generation-2))));
    number_of_females(generation)=round(max_females/(1+((max_females-((JPT_male_bottleneck+JPT_female_bottleneck)/2))/((JPT_male_bottleneck+JPT_female_bottleneck)/2))*exp(-population_growth_rate(generation)*(generation-JPT_settlement_generation-2))));
end
...
for i=1:1:runs
    ...
    % 1st element in each population state vector = # of monomorphic sites
    % 2nd element in each population state vector = # of singletons
    % 3rd element in each population state vector = # of doubletons
    % etc.
    ...
    for generation=1:1:max_generation-1
        ...
        % this means that empirical data needs to use corrected theta_w and pi
...
output_matrix=dataset({output_matrix 'mean','percentile_2pt5','median','percentile_97pt5'}, ...
              'obsnames', {'theta_w','pi'});
output_filename=['JPT_X_output_' int2str(JPT_male_bottleneck+2*JPT_female_bottleneck) 'Xchrom_' int2str(runs) 'runs.txt'];
