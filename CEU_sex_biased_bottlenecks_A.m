...
max_males=10000;
max_females=10000;
CEU_male_bottleneck=300;
CEU_female_bottleneck=200;
...
number_of_males(2)=CEU_male_bottleneck;
number_of_females(2)=CEU_female_bottleneck;
for generation=3:1:max_generation
    number_of_males(generation)=round(max_males/(1+((max_males-((CEU_male_bottleneck+CEU_female_bottleneck)/2))/((CEU_male_bottleneck+CEU_female_bottleneck)/2))*exp(-population_growth_rate(generation)*(generation-2))));
    number_of_females(generation)=round(max_females/(1+((max_females-((CEU_male_bottleneck+CEU_female_bottleneck)/2))/((CEU_male_bottleneck+CEU_female_bottleneck)/2))*exp(-population_growth_rate(generation)*(generation-2))));
end
...
output_matrix=dataset({output_matrix 'mean','percentile_2pt5','median','percentile_97pt5'}, ...
              'obsnames', {'theta_w','pi'});
output_filename=['CEU_A_output_' int2str(2*CEU_male_bottleneck+2*CEU_female_bottleneck) 'autosomes_' int2str(runs) 'runs.txt']
